...
Elm.Main.make = function (_elm) {
   ...
   var toLS = $Signal.channel("");
   var refreshStorage = _P.portOut("refreshStorage",
   _P.outgoingSignal(function (v) {
      return v;
   }),
   $Signal.subscribe(toLS));
   var formatDate = function (d) {
      return $String.join(":")(_L.fromArray([A2($String.join,
                                            "-",
                                            _L.fromArray([$Basics.toString($Date.year(d))
                                                         ,$Basics.toString($Date.month(d))
                                                         ,$Basics.toString($Date.day(d))]))
                                            ,A2($String.join,
                                            "-",
                                            _L.fromArray([$Basics.toString($Date.hour(d))
                                                         ,$Basics.toString($Date.minute(d))
                                                         ,$Basics.toString($Date.second(d))]))]));
   };
   var liKey = function (str) {
      return A2($Html.li,
      _L.fromArray([]),
      _L.fromArray([$Html.text(str)]));
   };
   var vToHtml = F2(function (v,
   takes) {
      return function () {
         switch (v.ctor)
         {case "EV1":
            return function () {
                 var str = A2($Json$Encode.encode,
                 2,
                 v._0);
                 var len = $String.length(str);
                 var n = A2($Maybe.withDefault,
                 len,
                 takes);
                 return $Html.text(A2($String.left,
                 n,
                 str));
              }();
            case "EV2": return A2($Html.ul,
              _L.fromArray([]),
              A2($List.map,liKey,v._0));
            case "EV3":
            return $Html.text(A2($Basics._op["++"],
              "price: ",
              $Basics.toString(v._0.price)));}
         return $Html.text("unknown value");
      }();
   });
   var btn = F2(function (name,
   handler) {
      return A2($Html.a,
      _L.fromArray([$Html$Attributes.id(name)
   ...
   var recordStyle = function (item) {
      return function () {
         var _v4 = item.selected;
         switch (_v4.ctor)
         {case "Just": switch (_v4._0)
              {case true:
                 return $Html$Attributes.$class("collection-item active");}
              break;}
         return $Html$Attributes.$class("collection-item");
      }();
   };
   var isPastFlag = function (b) {
      return function () {
         switch (b)
         {case false:
            return $Html.text("✓");
   ...
   var tHeader = A2($Html.tr,
   _L.fromArray([$Html$Attributes.$class("collection")]),
   _L.fromArray([A2($Html.th,
                _L.fromArray([]),
                _L.fromArray([$Html.text("Key")]))
                ,A2($Html.th,
                _L.fromArray([]),
                _L.fromArray([$Html.text("Value")]))
                ,A2($Html.th,
                _L.fromArray([]),
                _L.fromArray([$Html.text("Expire")]))]));
   var viewSelected = function (m) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("card-panel teal")]),
      _L.fromArray([A2($Html.span,
      _L.fromArray([$Html$Attributes.$class("white-text")]),
      _L.fromArray([A2(vToHtml,
      m.value,
      $Maybe.Nothing)]))]));
   };
   var isSelected = function (m) {
      return _U.eq(m.selected,
      $Maybe.Just(true));
   };
   ...
   xs) {
      return function () {
         ...
         {case "RowClick":
            return function () {
                 var updateSelection = function (t) {
                    return _U.eq(t.key,
                    action._0) ? _U.replace([["selected"
                                             ,$Maybe.Just(true)]],
                    t) : t;
                 };
                 return A2($List.map,
   ...
   var toDate = function (str) {
      return function () {
         var _v9 = $Date.fromString(str);
         switch (_v9.ctor)
         {case "Err":
            return $Maybe.Nothing;
            case "Ok":
            return $Maybe.Just(_v9._0);}
         _U.badCase($moduleName,
         "between lines 65 and 67");
      }();
   };
   var RowClick = function (a) {
      return {ctor: "RowClick"
             ,_0: a};
   };
   var NoOp = {ctor: "NoOp"};
   var updates = $Signal.channel(NoOp);
   var makeLi = function (model) {
      return A2($Html.tr,
      _L.fromArray([recordStyle(model)
                   ,$Html$Events.onClick(A2($Signal.send,
                   updates,
                   RowClick(model.key)))]),
      _L.fromArray([A2($Html.td,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text(model.key)]))
                   ,A2($Html.td,
                   _L.fromArray([$Html$Attributes.$class("hide-on-small-only")]),
                   _L.fromArray([A2(vToHtml,
                   model.value,
                   $Maybe.Just(100))]))
                   ,A2($Html.td,
                   _L.fromArray([]),
                   _L.fromArray([isPastFlag(model.isPast)
                                ,$Html.text(formatDate(model.expire))]))]));
   };
   var view = function (items) {
      return function () {
         var xs = items;
         var s = A2($List.filter,
         isSelected,
         xs);
         return $Html.div(_L.fromArray([$Html$Attributes.$class("row main")]))(_L.fromArray([A2($Html.div,
                                                                                            ...
                                                                                            _L.fromArray([A2(btn,
                                                                                                         ...
                                                                                                         "clear"))]))
                                                                                            ,A2($Html.table,
                                                                                            _L.fromArray([$Html$Attributes.$class("collection responsive-table striped")]),
                                                                                            A2($List._op["::"],
                                                                                            tHeader,
                                                                                            A2($List.map,
                                                                                            makeLi,
                                                                                            xs)))
                                                                                            ,A2($Html.div,
                                                                                            _L.fromArray([$Html$Attributes.$class("stick-bottom")]),
                                                                                            A2($List.map,
                                                                                            viewSelected,
                                                                                            s))]));
      }();
   };
   var Manifest = F2(function (a,
   b) {
      return {_: {}
             ,pub: a
             ,pubZone: b};
   });
   var manifestDecoder = A3($Json$Decode.object2,
   Manifest,
   A2($Json$Decode._op[":="],
   "pub",
   $Json$Decode.string),
   ...
   var offerDecoder = A2($Json$Decode.object1,
   Offer,
   A2($Json$Decode._op[":="],
   "price",
   $Json$Decode.$int));
   ...
   var infoDecoder = A4($Json$Decode.object3,
   Info,
   A2($Json$Decode._op[":="],
   "ip",
   $Json$Decode.string),
   ...
   $Json$Decode.string));
   var EV3 = function (a) {
      return {ctor: "EV3",_0: a};
   };
   var EV2 = function (a) {
      return {ctor: "EV2",_0: a};
   };
   var EV1 = function (a) {
      return {ctor: "EV1",_0: a};
   };
   var valueDecoder = $Json$Decode.oneOf(_L.fromArray([A2($Json$Decode.object1,
                                                      EV2,
   ...
   var entryDecoder = A7($Json$Decode.object6,
   Model,
   ...
   var initialModel = A2($Signal.map,
   decodeValues,
   getStorage);
